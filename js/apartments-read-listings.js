var card_listings = new Promise(function(resolve, reject) {
    ...
    properties.orderByChild("property_category").equalTo("Entire Place/Apartment").once("value", (snapshot) => {
        if (snapshot.exists()) {
...
card_listings.then(function() {
    ...
    ref.orderByChild("property_category").equalTo("Entire Place/Apartment").once("value", (snapshot) => {
        // var content = "";
        snapshot.forEach((childSnapshot) => {
            var val = childSnapshot.val();
            var childKey = childSnapshot.key;
